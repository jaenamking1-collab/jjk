...
- `Code.gs` is the source of the deployed Apps Script. **Editing it here does NOT deploy it.** Sync it with `clasp push` (configured — see below), or by pasting into the Apps Script editor. Keep `Code.gs` in sync with what's deployed.
  ...
  - **Redeploy with clasp**: `clasp list-deployments` → the live one is `AKfycbwJS1Fd…`; `clasp redeploy AKfycbwJS1Fd… -d "<note>"` cuts a new version and points that deployment at it. This is outward-facing (the public 분배금공지 page uses the same URL), so confirm with the owner first.
