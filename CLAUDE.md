...
- `Code.gs` is the source of the deployed Apps Script. **Editing it here does NOT deploy it.** Sync it with `clasp push` (configured — see below), or by pasting into the Apps Script editor. Keep `Code.gs` in sync with what's deployed.
  ...
  - **`clasp push` updates the editor content, not the deployment.** That is exactly the usual need: **time-driven triggers and manual `▶` runs use the saved editor code, so they take effect immediately with no redeploy.** Only the web app (`doGet`/`doPost` served at the `/exec` URL) keeps serving the old version until a new deployment is made — so redeploy only when a change touches the backend action contract.
  - One-time per machine: `clasp login`, and the **Apps Script API** toggle must be on at `script.google.com/home/usersettings`. Both are Google-account actions the owner performs.
