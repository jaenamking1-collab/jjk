...
function _solParsePost(logNo) {
  ...
  // 공지 직후엔 분배율이 아직 '업데이트 예정'(숫자 미입력)일 수 있다 → 그 경우에도 종목·예상분배금은 띄우고 rate=null(화면엔 '-').
  // SOL이 분배율을 채우면 다음 파싱에서 소수 분기로 잡혀 자동 갱신된다.
  const seg = txt.split(/분배금\s*내역/)[1] || '';
  const items = [];
  const re = /(?:^|\s)\d{1,2}\s+(SOL\s.+?)\s+([\d,]+)\s+(?:(\d{1,3}\.\d{1,2})|업데이트\s*예정)(?=\s|$)/g;
  let m;
