...
function _solParsePost(logNo) {
  ...
  return { sched, items: _solItems(txt) };
}

// 본문 텍스트 → 종목 행. 표기가 회차마다 달라 여기만 따로 뗐다(tools/test_sol_items.js 가 이 함수를 점검한다).
// ⚠️ SOL은 표기를 자주 바꾸고, 바뀌면 그 회차가 **통째로** 사라진다(0건이면 호출측이 버린다).
//   ① 월중 공지는 분배율 칸을 비운다 → 분배율은 선택.
//   ② 월말 공지는 분배율에 %가 붙는다('0.28%') → 숫자만 남기고 뗀다.
