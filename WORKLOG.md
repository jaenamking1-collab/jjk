...
- **수정(dist_notice.html)**:
  ...
  - ④-3 (추가) 탭 메뉴 신설: 제목 아래 sticky 탭바(달력·분배금공지·분배일정·종목전체), `showSection(name)`이 한 섹션만 표시(기본 달력). 모바일서 계속 스크롤 안 해도 됨. + 종목 카드 내부 스크롤(max-height) 모바일서 제거 → 카드 안에 스크롤 갇히던 문제 해결.
