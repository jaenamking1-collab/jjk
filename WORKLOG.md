...
- **다음 할 일(재남 손수)**: ① 공개 프록시 GAS에 새 `public_dist_proxy.gs` 붙여넣고 **재배포**(그래야 방문자수 뜸) ② 수정한 `dist_notice.html`을 **공개 저장소(jjk-dist)에 반영/배포**해야 실제 페이지에 적용됨.
- **URL 개인이름 문제**: `jaenamking1-collab.github.io/...`에 실명 계정명 노출 건 → 무료로 바꾸는 법 대화에서 안내(넷리파이/클라우드플레어 Pages 등).
