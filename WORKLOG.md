...
- **자동 탐지**: `refreshAllDistributions`(트리거용)가 `checkAndLogAlerts()`를 호출하도록 변경. 얘가 6개 운용사를 force 갱신하면서 **공시일이 직전과 달라지면 '신규공지' 알림**을 `알림로그` 시트에 남긴다 → SOL 새 글이 뜨면 자동 감지. 별도 트리거 추가 없음.
- **검증**: RSS 로컬 파싱 — item 50건, 중순① `7월 10일`(→공시일), 월말② `지급 예정일 7월 1일` 정상 추출.
- ⚠️ **Code.gs 재배포 필요.** 재배포 후 `?action=getDistribution&source=sol&force=1` 응답 `schedule.공시일`이 `7월 10일`이면 성공. 프론트는 안 건드렸으므로 jjk-dist 복사 불필요.
